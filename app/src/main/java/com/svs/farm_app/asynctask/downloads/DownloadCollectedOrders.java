...
public class DownloadCollectedOrders extends Request<String> {
	protected static final String TAG = DownloadCollectedOrders.class.getSimpleName();
	private int method;
	private String url;
	...
	public DownloadCollectedOrders(int method, String url, DatabaseHandler db, Context ctx, ErrorListener listener) {
	...
	private void addData(String response) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				db.addCollectedOrder(list.get(i));
			}
