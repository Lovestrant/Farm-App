...
public class UploadFarmIncome extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFarmIncome.class.getSimpleName();
	private final List<FarmIncome> farmIncome;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFarmIncome(Context ctx, List<FarmIncome> farmIncome) {
		this.mContext = ctx;
		this.farmIncome = farmIncome;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFarmIncome(Context ctx, FarmIncome farmIncome) {
		this.mContext = ctx;
		this.farmIncome = DataUtils.makeSingleList(farmIncome);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FARM_INCOME).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(farmIncome).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.UPLOAD_FARM_INCOME, Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addFarmIncome(farmIncome.get(0));
