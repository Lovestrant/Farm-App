...
public class UploadFarmerTimes extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFarmerTimes.class.getSimpleName();
	private final List<FarmerTime> farmerTimeList;
	Context mContext;
	private String responseString = null;
	private boolean isOnline = false;

	public UploadFarmerTimes(Context ctx, List<FarmerTime> farmerTimeList) {
		this.mContext = ctx;
		this.farmerTimeList = farmerTimeList;
        DashBoardActivity.isUploadFinished(mContext);
    }

	public UploadFarmerTimes(Context ctx, FarmerTime farmerTime) {
		this.mContext = ctx;
		this.farmerTimeList = DataUtils.makeSingleList(farmerTime);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.FARMER_TIME_UPLOAD_URL).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(farmerTimeList).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.FARMER_TIME_UPLOAD_URL, Toast.LENGTH_LONG).show());
		}
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				if (isOnline) {
					...
					Log.i(TAG,"clearing");
					db.clearTable(db.TABLE_FARMER_TIMES);
				}
			} else {
				...
				if (isOnline) {
					db.addFarmerTimes(farmerTimeList.get(0));
