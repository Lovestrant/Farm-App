...
public class UploadFarmOtherCrops extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFarmOtherCrops.class.getSimpleName();
	private final List<FarmOtherCrops> farmOtherCrops;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFarmOtherCrops(Context ctx, List<FarmOtherCrops> farmOtherCrops) {
		this.mContext = ctx;
		this.farmOtherCrops = farmOtherCrops;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFarmOtherCrops(Context ctx, FarmOtherCrops farmOtherCrops) {
		this.mContext = ctx;
		this.farmOtherCrops = DataUtils.makeSingleList(farmOtherCrops);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FARM_OTHER_CROPS).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(farmOtherCrops).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.UPLOAD_FARM_OTHER_CROPS, Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addFarmOtherCrops(farmOtherCrops.get(0));
