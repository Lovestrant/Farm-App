...
public class DownloadCottonDeductions extends Request<String> {
	protected static final String TAG = DownloadCottonDeductions.class.getSimpleName();
	private int method;
	private String url;
	...
	public DownloadCottonDeductions(int method, String url, DatabaseHandler db, Context ctx, ErrorListener listener) {
	...
	private void addData(String response) {
		...
		try {
			...
			List<CottonDeduction> list = gson.fromJson(response, new TypeToken<List<CottonDeduction>>() {
			}.getType());

			for (int i = 0; i < list.size(); i++) {
				db.addCottonDeduction(list.get(i));
			}
