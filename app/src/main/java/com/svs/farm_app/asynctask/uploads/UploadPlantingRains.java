...
public class UploadPlantingRains extends AsyncTask <Void, Void, String>{

private static final String TAG = UploadPlantingRains.class.getSimpleName();
	private final List<PlantingRains> plantingRainsList;
	...
	public UploadPlantingRains(Context ctx,List<PlantingRains> plantingRainsList) {
		this.mContext = ctx;
		db = new DatabaseHandler(ctx);
		cd = new ConnectionDetector(ctx);
		this.plantingRainsList = plantingRainsList;
	}

	public UploadPlantingRains(Context ctx, PlantingRains plantingRainsList) {
		this.mContext = ctx;
		this.plantingRainsList = DataUtils.makeSingleList(plantingRainsList);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_PLANTING_RAINS).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(plantingRainsList).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.UPLOAD_PLANTING_RAINS, Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addPlantingRains(plantingRainsList.get(0));
