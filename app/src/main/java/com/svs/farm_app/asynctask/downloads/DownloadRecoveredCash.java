...
public class DownloadRecoveredCash extends Request<String> {
	protected static final String TAG = DownloadRecoveredCash.class.getSimpleName();
	private int method;
	private String url;
	...
	public DownloadRecoveredCash(int method, String url, DatabaseHandler db, Context ctx, ErrorListener listener) {
	...
	private void addData(String response) {
		...
		try {
			...
			List<RecoveredCash> list = gson.fromJson(response, new TypeToken<List<RecoveredCash>>() {
			}.getType());

			for (int i = 0; i < list.size(); i++) {
				db.addRecoveredCash(list.get(i));
			}
