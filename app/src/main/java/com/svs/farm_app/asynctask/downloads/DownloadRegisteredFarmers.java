...
public class DownloadRegisteredFarmers extends Request<String> {
	protected static final String TAG = DownloadRegisteredFarmers.class.getSimpleName();
	private int method;
	private String url;
	...
	public DownloadRegisteredFarmers(int method, String url, DatabaseHandler db,Context ctx, ErrorListener listener) {
	...
	private void addData(String response) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				db.addRegisteredFarmers(list.get(i));
			}
