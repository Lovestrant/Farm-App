...
public class DownloadSubVillages extends Request<String> {
	private static final String TAG = DownloadSubVillages.class.getSimpleName();
	private int method;
	private String url;
	...
	public DownloadSubVillages(int method, String url, DatabaseHandler db, Context ctx, ErrorListener listener) {
	...
	private void addData(String response) {
		...
		try {
			...
			List<SubVillage> list = gson.fromJson(response, new TypeToken<List<SubVillage>>() {
			}.getType());

			for (int i = 0; i < list.size(); i++) {
				db.addSubvillages(list.get(i));
			}
