...
public class UploadFingerFiveForm extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFingerFiveForm.class.getSimpleName();
	private final List<FingerFive> fingerFives;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFingerFiveForm(Context ctx, List<FingerFive> fingerFives) {
		this.mContext = ctx;
		this.fingerFives = fingerFives;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFingerFiveForm(Context ctx, FingerFive fingerFive) {
		this.mContext = ctx;
		this.fingerFives = DataUtils.makeSingleList(fingerFive);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FINGER_FIVE_FORM).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(fingerFives).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at 5 ", Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addFingerFive(fingerFives.get(0));
