...
public class UploadGerminations extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFarmAssMajor.class.getSimpleName();
	private final List<Germination> germinations;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadGerminations(Context ctx, List<Germination> germinations) {
		this.mContext = ctx;
		this.germinations = germinations;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadGerminations(Context ctx, Germination germination) {
		this.mContext = ctx;
		this.germinations = DataUtils.makeSingleList(germination);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_GERMINATIONS).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(germinations).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.UPLOAD_GERMINATIONS, Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addGermination(germinations.get(0));
