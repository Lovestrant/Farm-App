...
public class UploadFarmProduction extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFarmProduction.class.getSimpleName();
	private final List<FarmProduction> farmProductions;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFarmProduction(Context ctx, List<FarmProduction> farmProductions) {
		this.mContext = ctx;
		this.farmProductions = farmProductions;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFarmProduction(Context ctx, FarmProduction farmProduction) {
		this.mContext = ctx;
		this.farmProductions = DataUtils.makeSingleList(farmProduction);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FARM_PRODUCTION).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(farmProductions).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.UPLOAD_FARM_PRODUCTION, Toast.LENGTH_LONG).show());
		}
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				if (isOnline) {
					...
					Log.i(TAG, "clearing");
					db.clearTable(db.TABLE_FARM_PRODUCTION);
				}
			} else {
				...
				if (isOnline) {
					db.addFarmProduction(farmProductions.get(0));
