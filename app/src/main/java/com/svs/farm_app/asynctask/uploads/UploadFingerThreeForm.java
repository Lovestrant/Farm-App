...
public class UploadFingerThreeForm extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFingerThreeForm.class.getSimpleName();
	private final List<FingerThree> fingerThrees;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFingerThreeForm(Context ctx, List<FingerThree> fingerThrees) {
		this.mContext = ctx;
		this.fingerThrees = fingerThrees;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFingerThreeForm(Context ctx, FingerThree fingerThree) {
		this.mContext = ctx;
		this.fingerThrees = DataUtils.makeSingleList(fingerThree);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FINGER_THREE_FORM).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(fingerThrees).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at 3" , Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addFingerThree(fingerThrees.get(0));
