...
public class UploadFingerFourForm extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFingerFourForm.class.getSimpleName();
	private final List<FingerFour> fingerFours;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFingerFourForm(Context ctx, List<FingerFour> fingerFours) {
		this.mContext = ctx;
		this.fingerFours = fingerFours;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFingerFourForm(Context ctx, FingerFour fingerFour) {
		this.mContext = ctx;
		this.fingerFours = DataUtils.makeSingleList(fingerFour);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FINGER_FOUR_FORM).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(fingerFours).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at 4" , Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addFingerFour(fingerFours.get(0));
