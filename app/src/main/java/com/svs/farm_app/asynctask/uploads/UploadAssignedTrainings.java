...
public class UploadAssignedTrainings extends AsyncTask<Void, Void, String> {

    private static final String TAG = UploadAssignedTrainings.class.getSimpleName();
    private final List<AssignedTrainings> assignedTrainingsList;
    Context mContext;
    private String responseString = null;
    private boolean isOnline = false;

    public UploadAssignedTrainings(Context ctx, List<AssignedTrainings> AssignedTrainingsList) {
        this.mContext = ctx;
        this.assignedTrainingsList = AssignedTrainingsList;
        DashBoardActivity.isUploadFinished(mContext);
    }

    public UploadAssignedTrainings(Context ctx, AssignedTrainings AssignedTrainings) {
        this.mContext = ctx;
        this.assignedTrainingsList = DataUtils.makeSingleList(AssignedTrainings);
        isOnline = true;
    }
    ...
    protected String doInBackground(Void... params) {
        ...
        try {
            HttpURLConnection connection = (HttpURLConnection) new URL(Config.ASSIGN_TRAININGS_UPLOAD_URL).openConnection();
            ...
            String urlParameters = "data=" + new Gson().toJson(assignedTrainingsList).toString();
            ...
            handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.ASSIGN_TRAININGS_UPLOAD_URL, Toast.LENGTH_LONG).show());
        }
        ...
        try {
            ...
            if ((object.get("message").equals("ok"))) {
                if (isOnline) {
                    ...
                    Log.i(TAG, "clearing");
                    db.clearTable(db.TABLE_ASSIGNED_TRAININGS);
                }
            } else {
                ...
                if (isOnline) {
                    db.addAssignedTrainings(assignedTrainingsList.get(0));
