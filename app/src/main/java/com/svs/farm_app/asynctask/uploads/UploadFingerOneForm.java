...
public class UploadFingerOneForm extends AsyncTask<Void, Void, String> {

	private static final String TAG = UploadFingerOneForm.class.getSimpleName();
	private final List<FingerOne> fingerOnes;
	Context mContext;
	private String response = null;
	private boolean isOnline = false;

	public UploadFingerOneForm(Context ctx, List<FingerOne> fingerOnes) {
		this.mContext = ctx;
		this.fingerOnes = fingerOnes;
		DashBoardActivity.isUploadFinished(mContext);
	}

	public UploadFingerOneForm(Context ctx, FingerOne fingerOne) {
		this.mContext = ctx;
		this.fingerOnes = DataUtils.makeSingleList(fingerOne);
		isOnline = true;
	}
	...
	protected String doInBackground(Void... params) {
		...
		try {
			HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_FINGER_ONE_FORM).openConnection();
			...
			String urlParameters = "data=" + new Gson().toJson(fingerOnes).toString();
			...
			handler.post(() -> Toast.makeText(mContext, "Issue at Finger 1" , Toast.LENGTH_LONG).show());
		}
	...
	protected void onPostExecute(String response) {
		...
		try {
			...
			if ((object.get("message").equals("ok"))) {
				...
				if (isOnline) {
					db.addFingerOne(fingerOnes.get(0));
