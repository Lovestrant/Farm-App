...
public class UploadSignedDocs extends AsyncTask<Void, Void, String> {

    private static final String TAG = UploadSignedDocs.class.getSimpleName();
    private final List<SignedDoc> signedDocs;
    Context mContext;
    private String response = null;
    private boolean isOnline = false;

    public UploadSignedDocs(Context ctx, List<SignedDoc> signedDocs) {
        this.mContext = ctx;
        this.signedDocs = signedDocs;
        DashBoardActivity.isUploadFinished(mContext);
    }

    public UploadSignedDocs(Context ctx, SignedDoc signedDoc) {
        this.mContext = ctx;
        this.signedDocs = DataUtils.makeSingleList(signedDoc);
        isOnline = true;
    }
    ...
    protected String doInBackground(Void... params) {
        ...
        try {
            HttpURLConnection connection = (HttpURLConnection) new URL(Config.UPLOAD_SIGNATURES).openConnection();
            ...
            String urlParameters = "data=" + new Gson().toJson(signedDocs).toString();
            ...
            handler.post(() -> Toast.makeText(mContext, "Issue at " + Config.UPLOAD_SIGNATURES, Toast.LENGTH_LONG).show());
        }
    ...
    protected void onPostExecute(String response) {
        ...
        try {
            ...
            if ((object.get("message").equals("ok"))) {
                ...
                if (isOnline) {
                    db.addSignedDoc(signedDocs.get(0));
